        ...
        <div class="article-module sidebar">
          <h4 id="hide1">Category</h4>
            <ul class="list-unstyled" id="hidden1">
                <li><?php echo $this->Html->link(__('Technology'),array('controller' => 'articles','action' 
                     =>'index','?' => array('cat_id'=>1)));?> </li>
                <li><?php echo $this->Html->link(__('Science and Health'),array('controller' => 'articles',
                    'action' =>'index','?' => array('cat_id'=>2))); ?> </li>
                <li><?php echo $this->Html->link(__('Programming'),array('controller' => 'articles','action' 
                     =>'index_filter','?' => array('cat_id'=>4))); ?> </li>
                <li><?php echo $this->Html->link(__('Random Articles'),array('controller' => 'articles',
                    'action'=>'index','?' => array('cat_id'=>3)));?></li>
            </ul>
        </div>
        <div class="article-module sidebar">
          <h4 id="hide1">Popular Articles</h4>
            <?php if($articles): ?>
              <?php foreach ($articles as $article): ?>
                <ul class= "list-unstyled" id= "hidden1">
                    <li style="font-size: 16px;"><?php echo $this->Html->link($article['Article']['title'],array( 'controller' => 'articles','action' => 'view', $article['Article']['id']));?>
                      </li>
                    <li><?php echo $this->Html->tag('span',$this->Text->truncate($article['Article']['body'],100)); ?></li>
                </ul>
              <?php endforeach;?>
            <?php endif;?>
        </div>
              ...
              <?php foreach($articles as $article):?>
                <?php if($article['Article']['id'] == 3):?>               
                  <?php echo $this->Html->link($article['Article']['title'],array('controller' => 'articles','action' =>'view',$article['Article']['id']));?>
                    <small><?php echo $this->Time->format('F j, Y h:i A',$article['Article']['created']); ?>
                    </small>
                <?php endif; ?>
              <?php endforeach; ?>
            <?php endif;  ?>
